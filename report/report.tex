...
in a linked list, as the statement asks. Listing~\ref{lst:expand} shows the
method.

\begin{lstlisting}[caption={The expand method generates the twelve children.},label={lst:expand}]
...
reading the action of each node. Listing~\ref{lst:astar} shows the algorithm.

\begin{lstlisting}[style=pseudo,caption={The A* search loop, as implemented in solve.},label={lst:astar}]
